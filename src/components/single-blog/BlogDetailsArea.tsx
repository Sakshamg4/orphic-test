...
export default function BlogDetailsArea() {
  ...
  useEffect(() => {
    if (id) {
      const fetchPost = async () => {
        try {
          const response = await fetch(`https://blog.orphicsolution.com/wp-json/wp/v2/posts/${id}?_embed`);
  ...
  // Use fetched data or fallback to static (if no ID provided, though route usually requires it)
  // For the purpose of this task, we will render the fetched post if available.

  const title = post ? post.title.rendered : "Top 5 reasons to launch your product with a creative agency";
  const content = post ? post.content.rendered : null;
  const imageUrl = post?._embedded?.['wp:featuredmedia']?.[0]?.source_url || "assets/images/blog/b11.png";
  const date = post ? new Date(post.date).toLocaleDateString('en-GB', { day: 'numeric', month: 'long', year: 'numeric' }) : "20 June 2025";
  const category = post?._embedded?.['wp:term']?.[0]?.[0]?.name || "Creative Agency";
    ...
    <section className="wionabout-section1 wiondefault-bg">
      <div className="container">
        <div className="wionsection-title center max-w100">
          ...
          <h2 className="aos-init" data-aos-delay="500" data-aos="fade-up" dangerouslySetInnerHTML={{ __html: title }} />
        </div>
        <div className="row">
          <div className="col-lg-8">
            <div className="blog-page-wrap aos-init" data-aos-delay="700" data-aos="fade-up">
              ...
              {content ? (
                <div className="wionblog-single-data mt-30" dangerouslySetInnerHTML={{ __html: content }} />
              ) : (
                <>
